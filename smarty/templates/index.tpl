	       	...
	       	<div class="box_content">
				<table class="document_list" id="my_version_feed">
					{section name=i loop=$my_recent_version_feed}
						<tr><td><img src="{$my_recent_version_feed[i].iconPtr}"></td><td>{$my_recent_version_feed[i].displayName} saved a version of </td><td><a href="{$my_recent_version_feed[i].link}"><p class="no_line_height">{$my_recent_version_feed[i].dName} {$my_recent_version_feed[i].vName}</p></a></td><td><p class="time small_text no_line_height" id="{$my_recent_version_feed[i].timestamp}">{$my_recent_version_feed[i].timestamp}</p></td></tr>
					{/section}
						</table>
				</div><!-- end box content -->
			</div>
